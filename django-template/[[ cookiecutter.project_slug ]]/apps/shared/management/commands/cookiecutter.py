...
def select_by_index(message: str, choices: List[str]) -> str:
    while True:
        ...
        click.echo(click.style(f'[🍪] {selected_index} is not valid', fg='red'))
...
def select_list(message: str) -> List[str]:
    click.echo('[🍪] enter the values comma separated')
...
def prompt_inputs() -> None:
    ...
    click.echo(click.style('[🍪] to update the project run the following command:\npython management.py cookiecutter_run', fg='green'))
